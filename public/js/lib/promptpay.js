...
export function generatePromptPayPayload(target, { amount } = {}) {
  ...
  const payloadFields = [
    ...
    field(ID_POI_METHOD, POI_METHOD_DYNAMIC),
    field(
    ...
    field(ID_TRANSACTION_AMOUNT, numericAmount.toFixed(2))
  ];
...
export async function createPromptPayQrPreview({ promptPayId, amount, expiryMinutes = 15 }) {
  ...
  const expiresAt = new Date(createdAt.getTime() + Math.max(1, Number(expiryMinutes) || 15) * 60_000);
